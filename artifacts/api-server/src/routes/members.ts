...
import { supabase, splitName, deriveStatus } from "../lib/supabase.js";
import { CreateMemberBody } from "../lib/types.js";
...
// GET /members/:id/transactions — Admin: get all transactions for a specific member
router.get("/members/:id/transactions", async (req, res): Promise<void> => {
  const id = req.params.id;
  const page = Math.max(1, Number(req.query.page) || 1);
